...
export async function cityMetadata(slug: string): Promise<Metadata> {
  ...
  const canonical = `${SITE_URL}${cityPath(slug)}`;
  return {
    title: `Events in ${city.name} This Weekend & Today | Odisha Event Alert`,
    description,
...
export default async function CityLanding({ slug }: { slug: string }) {
  ...
  const jsonLd = [
    breadcrumbJsonLd([
      ...
      { name: `Events in ${city.name}`, url: `${SITE_URL}${cityPath(slug)}` },
    ]),
    faqJsonLd(faqs),
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-12">
        ...
        <div className="glass-panel rounded-3xl p-7 sm:p-8 space-y-7">
          <LinkChips
            ...
            links={otherCities.map((c) => ({ name: `Events in ${c.name}`, href: cityPath(c.slug) }))}
          />
          <LinkChips
