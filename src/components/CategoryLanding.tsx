...
export default async function CategoryLanding({ slug }: { slug: string }) {
  ...
  const jsonLd = [
    breadcrumbJsonLd([
      ...
      { name: `${category.name} in Odisha`, url: `${SITE_URL}${categoryPath(slug)}` },
    ]),
    faqJsonLd(faqs),
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-12">
        ...
        <div className="glass-panel rounded-3xl p-7 sm:p-8 space-y-7">
          ...
          <LinkChips
            ...
            links={otherCategories.map((c) => ({ name: c.name, href: categoryPath(c.slug) }))}
          />
          <LinkChips
            title="Events by city"
            links={cities.slice(0, 6).map((c) => ({ name: `Events in ${c.name}`, href: cityPath(c.slug) }))}
          />
